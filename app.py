...
cors = CORS(app, resources={
            r"/view/latest/sensor/data/public/*": {"origins": "*"}})
...
def get_latest_sensor_log():
    ...
    if outdoor_device is not None:
        ...
        response["external_device_log"] = get_latest_log(
            outdoor_device, node_addr=outdoor_device)
...
def group_logs(response,url_info):
    average = {}
    ...
    for index, device in enumerate(indoor_devices):
        ...
        parameters = log.get("params").split(",")
        if index == 0:
            average = log
        else :
            for parameter in parameters :
                if average.get(parameter) is not None :
                    average_value = average.get(parameter).get("value") 
                    current_value = log.get(parameter).get("value")
                    calculated_value = round((average_value + current_value) / 2,2)
                    average[parameter] = { 
                        "value" : calculated_value , 
                        "incident_level":0
                    }

    response["data_logs"] = [average]
